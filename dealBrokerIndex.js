 ...
 * Body: { buyerId, sellerId, description, estimatedAmount, buyerProfile, sellerProfile }
 */
 ...
 * Body: { party: 'buyer'|'seller', message, counterTerms }
 */
app.post('/v1/deals/:dealId/respond', async (req, res) => {
  try {
    ...
    // Log the party's response
    db.logNegotiation(dealId, party, message, '');

    // AI negotiates response
 ...
 * Body: { paymentMethod: 'paypal', buyerEmail, sellerEmail, finalAmount }
 */
app.post('/v1/deals/:dealId/close', async (req, res) => {
  try {
    ...
    // Create PayPal payment for commission
    ...
    res.json({
      ...
      approvalUrl,
      closingMessage,
      message: 'Deal closed! Commission payment initiated. Buyer needs to approve PayPal payment.',
    });
  } catch (err) {
